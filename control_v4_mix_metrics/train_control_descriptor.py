...
def sample_eval_batch(diffusion, denoiser, control_net, batch, device, n_samples=4, eval_timesteps=1000, resample_jumps=2,
    ...
    """
    # DESCRIPTOR-AWARE wrapper. The base DynamicControlledDenoiser has no descriptor_field
    # parameter, so sampling through it leaves the field at None -> use_film=False -> the model
    # generates UNCONDITIONED. The panels then show a generic style next to a GT column of whatever
    # oracle the batch happened to hold, which looks like the model ignoring the target when in fact
    # it was never told what the target was.
