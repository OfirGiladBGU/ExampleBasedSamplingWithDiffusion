...
from control_v4_mix_metrics.DynamicControlNetDescriptor import (
    DynamicControlNetDescriptor, DynamicControlledDescriptorDenoiser,
)
from control_v4_mix_metrics.OracleStippleDataset import (
    OracleStippleDataset, list_image_stems, split_images,
)
from control_v4_mix_metrics.descriptor_fields import CONDITIONING_KEYS
from control_v4_mix_metrics import soft_descriptors as SD
...
# ── multi-oracle descriptor conditioning (M2) ────────────────────────────
# One model over every oracle, conditioned on the MEASURED descriptor field rather than an
# oracle label, so intermediate values are meaningful and control can vary across the image.
ORACLE_ROOT = "/groups/asharf_group/ofirgila/ControlNet/training/Icons-50_1024_Oracles"
ORACLES = "gbn,wvs,bnot,fs,ordered,white,jitgrid"
DESCRIPTOR_INJECT = "film"      # film | hint | both  (see DynamicControlNetDescriptor)
DESCRIPTOR_HIDDEN = 64

# ── descriptor-consistency loss (the "close the loop" variant) ───────────
# Measures the descriptor the model ACTUALLY produced against the one it was asked for,
# instead of relying on imitation to form that association implicitly.
#
# Default keys are COORDS-ONLY on purpose. nn_cv / aniso / pcf_peak need nothing but the
# decoded point positions, so there is no risk of the loss disagreeing with the precomputed
# conditioning over an image-preprocessing convention. edge_align calibrated EXACT too, but it
# additionally needs rho and a Sobel map, and those must match how precompute_descriptors built
# them or the term optimises a slightly different quantity than it is judged against. It is
# opt-in until that is verified end to end.
DESCRIPTOR_LOSS_WEIGHT = 0.0        # 0 disables; calibrate via the grad-ratio log
# nn_cv and aniso only: both calibrated EXACT (r = 1.00000 / 0.99863) with matching oracle rank,
# and both gradient-safe. pcf_peak is deliberately excluded -- faithful values, pathological
# gradients (see soft_descriptors). It remains a CONDITIONING channel either way.
DESCRIPTOR_LOSS_KEYS = "nn_cv,aniso"
DESCRIPTOR_LOSS_T_FRAC = 0.4        # low-t only: a descriptor on a noisy x0 measures noise
DESCRIPTOR_LOSS_WARMUP_EPOCHS = 5
DESCRIPTOR_PCF_SIGMA = 0.08         # from sweep_pcf_sigma.py: r=0.9996, oracle rank MATCH
...
def sample_eval_batch(diffusion, denoiser, control_net, batch, device, n_samples=4, eval_timesteps=1000, resample_jumps=2,
    ...
    """
    # DESCRIPTOR-AWARE wrapper -- see train_control_descriptor.py. Sampling through the base
    # wrapper leaves descriptor_field=None, so the eval panels are generated UNCONDITIONED.
    controlled = DynamicControlledDescriptorDenoiser(denoiser, control_net)
    high_res_img = batch["high_res"]
    descriptor = batch.get("descriptor")
    ...
    controlled.set_condition(high_res_img, high_res_sdf, target_density, target_sdf,
                             smart_init_grid, descriptor_field=descriptor)
...
def dynamic_collate(batch):
    ...
    # Descriptor conditioning. This collate is an explicit WHITELIST -- anything not named here is
    # silently dropped, which is exactly how the descriptor field went missing: the model received
    # descriptor_field=None, FiLM never engaged, and the run trained completely unconditioned while
    # reporting a perfectly healthy loss. `oracle` is a str and cannot be stacked; it is carried as
    # a plain list for logging.
    if "descriptor" in sample0:
        collated["descriptor"] = torch.stack(
            [sample["descriptor"].contiguous() for sample in batch], dim=0)
    if "descriptor_valid" in sample0:
        collated["descriptor_valid"] = torch.stack(
            [sample["descriptor_valid"].contiguous() for sample in batch], dim=0)
    if "oracle" in sample0:
        collated["oracle"] = [sample["oracle"] for sample in batch]
...
def main():
    ...
    parser.add_argument("--oracle-root", default=ORACLE_ROOT,
                        help="dataset root holding target_<m>/ and descriptors_<m>/")
    parser.add_argument("--oracles", default=ORACLES,
                        help="comma-separated oracle names to train over")
    parser.add_argument("--descriptor-inject", default=DESCRIPTOR_INJECT,
                        choices=("film", "hint", "both"))
    parser.add_argument("--descriptor-hidden", type=int, default=DESCRIPTOR_HIDDEN)
    parser.add_argument("--descriptor-loss-weight", type=float, default=DESCRIPTOR_LOSS_WEIGHT,
                        help="weight of the descriptor-consistency term (0 = off)")
    parser.add_argument("--descriptor-loss-keys", default=DESCRIPTOR_LOSS_KEYS,
                        help="subset of " + ",".join(SD.SOFT_KEYS))
    parser.add_argument("--descriptor-loss-t-frac", type=float,
                        default=DESCRIPTOR_LOSS_T_FRAC)
    parser.add_argument("--descriptor-loss-warmup-epochs", type=int,
                        default=DESCRIPTOR_LOSS_WARMUP_EPOCHS)
    parser.add_argument("--descriptor-pcf-sigma", type=float, default=DESCRIPTOR_PCF_SIGMA)
    parser.add_argument("--descriptor-loss-grad-log-every", type=int, default=2000)
    parser.add_argument("--descriptor-loss-probe", action=argparse.BooleanOptionalAction,
                        default=False,
                        help="Measure the descriptor/denoise gradient ratio but do NOT add the term "
                             "to the loss. Use this to pick --descriptor-loss-weight: an "
                             "uncalibrated weight can destabilise training before you have read a "
                             "single ratio, and a probe run cannot.")
    parser.add_argument("--descriptor-stats", default=None,
                        help="DESCRIPTOR_STATS.json (default: <oracle-root>/DESCRIPTOR_STATS.json)")
...
def run(args):
    ...
    control_net = DynamicControlNetDescriptor(
        denoiser,
        descriptor_channels=len(CONDITIONING_KEYS),
        descriptor_hidden=args.descriptor_hidden,
        descriptor_inject=args.descriptor_inject,
    ...
    # ── multi-oracle dataset ─────────────────────────────────────────────
    # One (image, oracle) pair per sample. Offsets are per-oracle, so ensure_offsets_dir runs once
    # per target_<m>/ -- it prefers each target's exact <stem>.npy over PNG centroid detection
    # (POINTS_SOURCE), which matters most for the clumpy oracles whose PNGs lose points to pixel
    # adjacency.
    oracles = [m.strip() for m in args.oracles.split(",") if m.strip()]
    stats_path = args.descriptor_stats or os.path.join(args.oracle_root, "DESCRIPTOR_STATS.json")
    if not os.path.exists(stats_path):
        raise RuntimeError(
            "descriptor stats not found: " + stats_path + "\n"
            "Run: python control_v4_mix_metrics/precompute_descriptors.py --stage all"
        )

    # descriptor-consistency setup: the loss must live in the SAME normalised space as the
    # conditioning, so it reads lo/hi from the same stats file the dataset uses.
    _stats_all = json.load(open(stats_path))["descriptors"]
    desc_loss_keys = [k.strip() for k in args.descriptor_loss_keys.split(",") if k.strip()]
    _bad = [k for k in desc_loss_keys if k not in SD.SOFT_KEYS]
    if _bad:
        raise ValueError(f"no differentiable implementation for {_bad}; available {SD.SOFT_KEYS}")
    desc_loss_stats = {k: (_stats_all[k]["lo"], _stats_all[k]["hi"]) for k in desc_loss_keys}
    desc_loss_chan = [list(CONDITIONING_KEYS).index(k) for k in desc_loss_keys]
    desc_loss_enabled = args.descriptor_loss_weight > 0.0
    n_desc_nonfinite = 0
    desc_t_cutoff = int(truncation_cutoff * args.descriptor_loss_t_frac)
    print(f"Descriptor-consistency loss    : "
          + (f"on (w={args.descriptor_loss_weight}, keys={desc_loss_keys}, "
             f"t<{desc_t_cutoff}, pcf_sigma={args.descriptor_pcf_sigma}"
             f"{', PROBE: measured but NOT applied' if args.descriptor_loss_probe else ''})"
             if desc_loss_enabled else "off"))

    oracle_offsets = {}
    for m in oracles:
        target_dir = os.path.join(args.oracle_root, "target_" + m)
        if not os.path.isdir(target_dir):
            raise RuntimeError("missing oracle target dir: " + target_dir)
        print("\n[oracle " + m + "] offsets")
        oracle_offsets[m] = ensure_offsets_dir(
            args.source, target_dir,
            os.path.join(args.oracle_root, "offsets_" + m),
            args.grid_size, points_source=args.points_source,
        )

    # Split BY SOURCE IMAGE, reproducing train_control.py's randperm(seed=42) + int(n*val_split)
    # tail exactly, so an image that is validation for a control_v4 run is validation here too.
    # Every oracle of a held-out image goes to val together: descriptor generalisation and image
    # generalisation are separate axes and must not be conflated.
    all_filenames = list_image_stems(args.source, require_dir=oracle_offsets[oracles[0]])
    if not all_filenames:
        raise RuntimeError(
            "no images with offsets under " + args.source + ". Ensure source images and offsets "
            "share matching relative stems (including subfolders)."
        )
    train_filenames, val_filenames = split_images(all_filenames, val_split=args.val_split, seed=42)
    ...
    train_dataset = OracleStippleDataset(
        args.source, oracle_offsets, args.oracle_root, stats_path, train_filenames,
        grid_size=args.grid_size, **ds_kwargs)
    print("\n" + train_dataset.summary())

    val_dataset = None
    if val_len > 0:
        val_dataset = OracleStippleDataset(
            args.source, oracle_offsets, args.oracle_root, stats_path, val_filenames,
            grid_size=args.grid_size, **ds_kwargs)
    ...
    print("\nImages: train=%d, val=%d  ->  samples: train=%d, val=%d over %d oracles"
          % (train_len, val_len, len(train_dataset), n_val_samples, len(oracles)))
    ...
    # The descriptor-consistency loss also decodes offsets -> points: every descriptor is a
    # function of point POSITIONS. Omitting it here left grid_centers_flat = None and crashed the
    # first step whenever C2 and smart-init were both off.
    _need_point_centers = (args.smart_init_features or density_loss_enabled
                           or desc_loss_enabled)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_pbar:
            ...
            if descriptor is None:
                # Fail loudly. A descriptor-conditioned trainer with no descriptor silently
                # degrades to an unconditioned one and still reports a healthy loss -- the most
                # expensive kind of bug, because nothing looks wrong.
                raise RuntimeError(
                    "batch has no 'descriptor': the conditioning never reaches the model and "
                    "FiLM stays disabled. Check dynamic_collate passes it through.")
            high_res_sdf = batch.get("high_res_sdf")
            target_sdf = batch.get("target_sdf")
            smart_init_grid = batch.get("smart_init_grid")
            smart_init_offsets = batch.get("smart_init_offsets")
            ...
            desc_loss_item = 0.0
            if desc_loss_enabled and descriptor is not None:
                desc_w = args.descriptor_loss_weight
                if args.descriptor_loss_warmup_epochs > 0:
                    desc_w *= min(1.0, float(epoch) / float(args.descriptor_loss_warmup_epochs))
                # Low-t only. At high t the decoded x0 is mostly noise, and its "descriptor" is a
                # statistic of that noise -- exactly why the C2 density loss is masked the same way.
                sel = (t < desc_t_cutoff)
                if desc_w > 0.0 and bool(sel.any()):
                    x0_pred_d = diffusion.predict_xstart_from_noise(offsets_t, t, noise_pred)
                    coords_d = offsets_to_coords_gpu(x0_pred_d[sel], args.grid_size,
                                                     grid_centers_flat)
                    req = descriptor[sel][:, desc_loss_chan]
                    dc_loss = SD.descriptor_consistency_loss(
                        coords_d, None, req,
                        keys=desc_loss_keys, stats=desc_loss_stats,
                        G=args.grid_size, sigma_bins=args.descriptor_pcf_sigma,
                    )
                    if (args.descriptor_loss_grad_log_every > 0
                            and global_step % args.descriptor_loss_grad_log_every == 0):
                        _gp = [p for p in control_net.parameters() if p.requires_grad]
                        if not args.freeze_denoiser:
                            _gp += [p for p in denoiser.parameters() if p.requires_grad]
                        gn_main = loss_grad_norm(denoise_loss, _gp)
                        gn_desc = loss_grad_norm(desc_w * dc_loss, _gp)
                        _ratio = gn_desc / max(gn_main, 1e-12)
                        print(f"  [DESC grad] step {global_step}: |g_main|={gn_main:.4e} "
                              f"|g_desc|={gn_desc:.4e} ratio={_ratio:.3f} "
                              f"(target 0.05-0.20) active={int(sel.sum())}/{len(sel)}")
                        if use_wandb:
                            wandb.log({"desc/grad_ratio": _ratio,
                                       "desc/grad_main": gn_main,
                                       "desc/grad_desc": gn_desc}, step=epoch + 1)
                    if not torch.isfinite(dc_loss):
                        n_desc_nonfinite += 1
                        if n_desc_nonfinite in (1, 10, 100) or n_desc_nonfinite % 1000 == 0:
                            print(f"  [DESC] non-finite loss at step {global_step} "
                                  f"(count={n_desc_nonfinite}); term skipped this step")
                    elif not args.descriptor_loss_probe:
                        loss = loss + desc_w * dc_loss
                        desc_loss_item = float(dc_loss.detach())
                    else:
                        desc_loss_item = float(dc_loss.detach())
        ...
        # FiLM activity: 0 at init by construction (zero-init gates). If it does NOT grow, the model
        # is ignoring the descriptor field -- the plan's "metric response without visual change"
        # failure, caught here in a log line instead of an eval panel weeks later.
        if descriptor is not None and hasattr(control_net, "film_activity"):
            try:
                act = control_net.film_activity(descriptor[:1])
                gates = [float(p.detach().abs().mean()) for n, p in control_net.named_parameters()
                         if n.endswith(".gate")]
                print("            |  film |scale| = %.4e  mean|gate| = %.4e"
                      % (act.get("mean_abs_scale", float("nan")),
                         sum(gates) / max(len(gates), 1)))
                if use_wandb:
                    wandb.log({"metrics/film_abs_scale": act.get("mean_abs_scale", 0.0),
                               "metrics/film_abs_gate": sum(gates) / max(len(gates), 1),
                               "epoch": epoch})
