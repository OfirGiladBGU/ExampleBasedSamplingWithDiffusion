"""Generate stipple point sets using the Dynamic ControlNet V2.

The DynamicControlledDenoiser is a drop-in replacement for the base
denoiser inside the diffusion sampling loop.  At every reverse-diffusion
step it dynamically recomputes the local density by sampling the
high-res image at the current estimated point positions.
...
def main():
    ...
    parser.add_argument("--control_ckpt", required=True,
                        help="Path to trained Dynamic ControlNet .pt file")
