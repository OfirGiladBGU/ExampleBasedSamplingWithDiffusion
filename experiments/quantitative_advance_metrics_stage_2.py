...
def parse_args():
    ...
    p.add_argument("--folders", nargs="+", default=RESULTS_DIR_LIST,
                   help="List of _json folders to aggregate; when provided, writes one combined summary JSON and exits")
    p.add_argument("--folders-summary-file", default=RESULTS_SUMMERY_FILE,
                   help="Output JSON filename for --folders mode, written under --output")
...
def aggregate_folder(folder: Path):
    """Read all JSONs under `folder` and compute mean and std for each numeric metric.

    Returns a dict mapping metric -> {"mean": float, "std": float, "count": int}
    """
    js = sorted([p for p in Path(folder).glob("*.json") if p.is_file()])
    if not js:
        return {}
...
def main():
    args = parse_args()
    # If folders mode is provided, process those folders and exit.
    if getattr(args, "folders", None):
        ...
        for i, f in enumerate(folders, start=1):
            # Resolve path: try as-is, otherwise relative to output base
            p = Path(f).expanduser()
            if not p.exists():
                p = out_base / f
            p = p.resolve()
            print(f"[{i}/{total}] Processing folder: {p.name}")
            if not p.exists() or not p.is_dir():
                print(f"  Skipping: not a directory: {p}")
                continue
            summary = aggregate_folder(p)
            combined[p.name] = summary

        out_path = out_base / args.folders_summary_file
