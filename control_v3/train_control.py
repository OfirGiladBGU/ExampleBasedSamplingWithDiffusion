"""Train the Dynamic ControlNet V3 for stipple generation.

V3 uses a wider hint encoder (21ch->128ch), GECCO-style feature
extraction, open gates (bias=0.0), and coordinate grid conditioning.
...
from control_v3.DynamicControlNet import DynamicControlNet
...
def main():
    ...
    parser.add_argument("--out", default="control_v3/control_out",
    ...
    if use_wandb:
        try:
            import wandb
            load_wandb_key()
            wandb.init(project="dynamic-controlnet-v3-stipple", config=vars(args))
        except ImportError:
