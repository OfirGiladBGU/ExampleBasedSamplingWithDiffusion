...
Usage (from project root):
    python control_gt_free/train_control.py \
        ...
        --batch_size 16 \
        --lr 1e-4 \
        --out control_gt_free/control_out
"""
...
OUTPUT_DIR = "control_gt_free/train_outputs_icons50_512_CONCAT"
GRID_SIZE = 32
VAL_SPLIT = 0.1
EPOCHS = 2000
SAVE_EVERY = 10
...
# OUTPUT_DIR = "control_gt_free/train_outputs_data_celeba_5K_1024_no_random"
# GRID_SIZE = 32
# VAL_SPLIT = 0.1
# EPOCHS = 10000
# SAVE_EVERY = 10
...
# OUTPUT_DIR = "control_gt_free/train_outputs_data_AM-2K_1024_no_random"
# GRID_SIZE = 32
# VAL_SPLIT = 0.1
# EPOCHS = 20000
# SAVE_EVERY = 10
...
# OUTPUT_DIR = "control_gt_free/train_outputs_data_stress1_no_random"
# GRID_SIZE = 32
# VAL_SPLIT = 0.0
# EPOCHS = 10000
# SAVE_EVERY = 50
...
# OUTPUT_DIR = "control_gt_free/train_outputs_data_stress2_V2_no_random"
# ENABLE_SMART_INIT_JITTER = False
# ENABLE_SMART_INIT_SPLAT_SIGMA = False
# GRID_SIZE = 32
# VAL_SPLIT = 0.0
...
# OUTPUT_DIR = "control_gt_free/train_outputs_data_stress2_no_random"
# GRID_SIZE = 32
# VAL_SPLIT = 0.0
# EPOCHS = 10000
# SAVE_EVERY = 10
...
def run(args):
    ...
    # -- auto-build the empirical PCF target (GBN bar) so the hybrid PCF term works flag-free --
    if args.gt_free and args.w_pcf > 0 and not args.gbn_bar:
        default_bar = os.path.join(args.out, "gbn_bar.pt")
        if not os.path.exists(default_bar):
            from control_gt_free.eval.precompute_gbn_bar import build_bar_from_dataset
            print(f"[train] no --gbn-bar; auto-building PCF target from the dataset -> {default_bar}")
            build_bar_from_dataset(args.source, args.offsets, default_bar,
                                   grid_size=args.grid_size, device=str(device))
        args.gbn_bar = default_bar
    ...
    if getattr(args, "gt_free", False):
        ...
        _tau_total = max(1, int(total_opt_steps))
        ...
        for batch in train_pbar:
            ...
            if getattr(args, 'gt_free', False) and args.gt_free_mode == "unroll":
                ...
                if conditioner is not None:
                    ...
                    _controlled.set_condition(high_res_img, high_res_sdf, target_density, target_sdf, smart_init_grid)
                ...
                _tau = tau_sched.value(global_step)
                ...
                if conditioner is not None:
                    ...
                    controls = control_net(
                        offsets_t, t_net, high_res_img, target_density,
                ...
                if getattr(args, 'gt_free', False):
                    ...
                    _lam = args.geo_weight if global_step >= args.geo_warmup else 0.0
                    if _lam > 0.0:
                        _tt = t.view(-1, 1, 1, 1)
                ...
                if cv_ok and clump_ok and geom_score < best_geom_score:
                    ...
                    best_payload = {
                        ...
                        "gtfree_config": {
                            ...
                            "concat_smart_init_grid": getattr(args, "concat_smart_init_grid", False),
