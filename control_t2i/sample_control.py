"""Generate stipple point sets using control_t2i lightweight adapter guidance."""
...
from control_t2i.smart_init import build_smart_init_from_image, save_smart_init_debug
...
INPUT_IMAGE_PATH = "control_v4/sample_test/gradient0deg.png"
GT_IMAGE_PATH = ""
OUTPUT_DIR = "control_t2i/sample_outputs"
N_SAMPLES = 1
TIMESTEPS = 1000
GRID_SIZE = 32
RESAMPLE_JUMPS = 2
...
def load_condition(image_path, grid_size, device):
    ...
    high_res = torch.from_numpy(image_01).unsqueeze(0).unsqueeze(0).to(device)  # (1,1,H,W)
    target_density = F.interpolate(high_res, size=(grid_size, grid_size), mode="area")
    return image_01, high_res, target_density
...
def _extract_control_state_dict(ctrl_state):
    ...
    for key in ("adapter", "control_net", "model_state_dict", "state_dict"):
    ...
    raise KeyError(
        ...
        f"Expected one of ['adapter', 'control_net', 'model_state_dict', 'state_dict']; found keys: [{keys_preview}]"
    )
...
def main():
    ...
    adapter.load_state_dict(_extract_control_state_dict(ctrl_state), strict=False)
    adapter.eval()
    ...
    image_01, high_res, target_density = load_condition(
        args.input_image,
        args.grid_size,
        device,
    )
    ...
    with torch.no_grad() if args.resample_jumps == 0 else torch.enable_grad():
        for i in tqdm(reversed(range(t_start)), total=t_start, desc="sampling_v5"):
